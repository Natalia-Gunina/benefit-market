...
### B. Уже применённые миграции

Workflow сам помечает `00001..00008` как applied при каждом запуске
(шаг "Bootstrap migration history"). Это идемпотентно — после первого
раза становится no-op.

Если в будущем понадобится пометить applied какие-то ещё версии (например,
если применены свежие миграции вручную), используй ручной запуск
workflow с полем `mark_applied`:
1. Открой <https://github.com/Natalia-Gunina/benefit-market/actions/workflows/db-migrate.yml>
2. **Run workflow** → в `mark_applied` через запятую: `00010, 00011`.

После единственного push в `main`, меняющего `supabase/migrations/**`,
